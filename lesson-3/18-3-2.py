...
path = input('Путь к файлу: ').lower()
disk = input('На каком диске должен лежать файл: ').lower()
ext = input('Требуемое расширение файла: ').lower()

if not path.startswith(disk):
    print('Ошибка ввода! Некорректно указан диск.')
elif not path.endswith(ext):
    print('Ошибка ввода! Неверное расширение')
else:
